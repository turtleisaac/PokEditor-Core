...
script_file : (WHITESPACE | NEWLINE)*? (label_definition | clean_syntax_command | command | action_definition | action_command /*| valid_table*/ )* (WHITESPACE | NEWLINE)*? EOF;
...
clean_syntax_command : (WHITESPACE*? (postfix_increment | postfix_decrement | assignment) NEWLINE) ;
postfix_increment : (NAME | NUMBER) WHITESPACE*? INCREMENT ;
postfix_decrement : (NAME | NUMBER) WHITESPACE*? DECREMENT ;
assignment : (NAME | NUMBER) WHITESPACE*? EQUALS WHITESPACE*? (NAME | NUMBER) WHITESPACE*? ;
